  ...
  return (
    <div className="scroll"><div className="page">
      ...
      <div className="cal">
        ...
        {data.days.map((d) => (
          <div key={d.date}
               title={[d.holiday, d.events && `日程 ${d.events}`, d.points && `考点推进 ${d.points}`, d.due && `待复习 ${d.due}${d.overdue ? `（含逾期 ${d.overdue}）` : ''}`, d.reviewed && `已复习 ${d.reviewed}`, d.created && `新建 ${d.created}`, d.words && `背词 ${d.words}`, d.exams && `做题 ${d.exams}`].filter(Boolean).join(' · ') || undefined}
               className={[
                 'day-cell',
                 d.isToday ? 'today' : '',
            ...
            <div className="dc-dots">
              ...
              {d.points > 0 && <i className="pt" title={`考点推进 ${d.points}`} />}
  ...
  return (
    <div className="scroll"><div className="page" style={{ maxWidth: 900 }}>
      ...
      {data.advances?.length > 0 && (
        <div style={{ marginTop: 40 }}>
          <Band title="考点推进" meta={`${data.advances.length} 个`}>
            {data.advances.map((e, i) => (
              <button key={`${e.group}/${e.name}/${e.stage}`} className="pt-row" onClick={() => e.noteId && navigate(`/note/${encodeURIComponent(e.noteId)}`)}>
                <i className="pt-sw" style={{ background: 'var(--accent)' }} />
                <span className="pt-name">{e.name}</span>
                <span className="pt-sub">{e.subject}</span>
                <span className="pt-r">→ {e.stageName}</span>
              </button>
            ))}
          </Band>
        </div>
