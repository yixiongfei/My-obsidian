...
按下去就定在那儿，定住之后不能移动、不能放缩、也撕不掉；把图钉拔掉，右上角露出一个翘起的纸角，
**按住它往左拖**，纸以右上角为轴掀起来、右边裂开锯齿、越拖越斜，拖过 84px 才真撕下来，
没拖够就弹回去。不做成「点一下就删」是有原因的：纸片上写的是自己想明白的过程，误删一张没地方找回来，
所以要一个明确的、带方向的动作。写字支持 `**粗体**`、`- 条目`、`> 小标题`，以及 `$x^2$` 行内公式——
