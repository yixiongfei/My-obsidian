...
| 仪表盘 | 倒计时、**今天该做什么**（从逾期考点、错题本、做过题没写笔记、连续天数里挑出最该动手的几件）、**考点进度**（今天学了哪些、多少要复习、每科走到哪）、**薄弱考点**（每本错题本记了几题，多的在前）、待复习笔记、**学习节奏**（近 26 周热力图 + 每日进度，笔记复习 / 背词 / 做题 / 新建同一份数据） |
...
取纸台最下面那个虚线格子是文件选择。纸片**严格按原图比例**建出来（等比缩到上限以内、太小再等比放大），
图铺满整张纸不留白边；图用正片叠底「印」在纸上——白底变成纸的颜色，字迹、荧光笔照旧，
黄纸上的截图就是黄的。点开看的大图不动，那是证据，一个像素都不改；正文里的图夜间会反相，纸片上的不会。
...
`web/src/sticky-tear.js` 是一份纯几何：**进去一个点 D，出来几条路径**，不碰 DOM、不管动画。

它模拟的不是「翻页」，是**剥一张背面涂满胶的纸**：捏住右上角 C 往左下拉到 D，胶从剥离线
开始一点点松开，松开的纸绕着一根平行于剥离线的圆柱弯起来，弯过顶点之后平平地翻回来，
一路伸到手指。纸上每个点用它到剥离线的有符号距离 t 归类：

```
      M ──────────────╮ C        t ≤ 0       还粘着，原样不动
      ┆ ░░░░░░░░░░░░░ │          0 < t ≤ θR  绕在半径 R 的圆柱上（θ 是纸角绕到的角度）
 D ·←─┆░░ 卷 ░░│      │          t > πR      绕过顶点，平铺在高度 2R 上往回伸
      ┆                │
      └────────────────┘         a：从 D 指向 C 的单位向量；剥离线过 M、垂直于 a
```

纸是**不可伸长**的，所以每一段的俯视投影都是刚性的：卷是 `p ↦ p + (R·sin(t/R) − t)·a`
（圆柱面压到平面上是一条正弦），平铺是 `p ↦ p − (2t − πR)·a`（正好等于关于直线 t = πR/2 做镜像）。
半径 R 固定为短边的三成——纸有硬度，撕起来是个大弧，不是紧紧一卷。刚开始拉的时候纸角只是
沿着这个弧往上爬（θ < π，由 θ − sin θ = L/R 解出，L 是 CD 的长度），爬过顶点之后多出来的纸
才翻平；两段里角 C 都精确落在 D 上，这是求剥离线位置的方程。

手指的位移先压回「往纸里拉」的象限（只取向左、向下的分量）：往右上拽是把纸扯离页面，
不是剥，剥离线因此永远从角落切进纸里，鼠标往上一抖不会让整张纸乱翻。

**为什么只动 D**：θ、剥离线、每一段的形状都是 D 的函数，各自插值会散架。只动 D，每帧现算，
形状永远自洽——没有内凹，没有拉伸。

落到 Web 上：字还看得见的部分（粘着的 + 正在抬起的正面）当 `clip-path: path()` 扣在便利贴
自己身上（**子元素、文字、图片、公式全都跟着被裁**）；抬起那段上面盖一层从剥离线到轮廓线
渐暗的半透明渐变，暗示它正在背光倾斜；绕过轮廓线露出的纸背和翻平的片是 SVG `<path>`，
必须是便利贴的**兄弟**而不是孩子——它们按定义就在裁剪区之外。

立体感来自四样东西，都不是贴图：纸背上一条从顶棱到轮廓线的 `linearGradient`（顶棱迎光最亮、
轮廓线最暗，凸面感全在这）、顶棱上一道细高光、轮廓线外侧一条模糊的接触阴影（卷贴着页面
的地方最暗），以及整块悬起的纸的 `drop-shadow`。

手感在 `Stickies.jsx` 里：纸角不是直接跟着鼠标，是隔着一根**弹簧**追——刚开始胶粘得牢，
弹簧很软；剥开一段之后胶松了，弹簧变硬；阻尼留一点余量，松手会微微回弹。拖过对角线 45%
松手才真撕下来，纸沿着当前方向一路卷到消失；没拖够就被胶慢慢拉回去贴平。
...
app/
  ...
  web/src/
    ...
    sticky-tear.js      剥离几何：一个控制点 D，推出剥离线、圆柱卷、翻平片（纯函数）
