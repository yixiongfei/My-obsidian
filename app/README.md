...
箭头记的是**内容，不是像素**（`web/src/sticky-anchor.js`）。落点在文字上时存的是
「元素路径 + 第几个文本节点 + 第几个字」，渲染时用 `Range` 量出那个字的矩形；落在公式、图片上时退一档，
存「元素路径 + 框内百分比」。所以折起目录树、拉窄窗口、图片晚加载一帧，箭头照样钉在那个字上——
存屏幕坐标的做法在第一次重排时就会指到空白处去。两种锚点都带一份绝对坐标和一小段原文兜底：
笔记被改到路径对不上，先按原文找回来，再找不到才退回绝对坐标。宁可指偏，不能让箭头凭空消失。
...
app/
  ...
  web/src/
    ...
    sticky-anchor.js    便利贴箭头的锚点：把落点记成「哪个字」而不是「哪个像素」
